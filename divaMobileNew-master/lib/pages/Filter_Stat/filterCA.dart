...
import '../../pages/ChiffreAffaireGS/widget/dropdown_payeur.dart';
...
import '../ChiffreAffaireGS/CA_CLI_MONT.dart';
...
class Filter_CA extends StatefulWidget {
  //late final ValueChanged<bool> update;
 // Filter_CA({required this.update});

  const Filter_CA({Key? key, }): super(key:key);
  @override
  State<Filter_CA> createState() => _ExampleAppPageState();
}

class _ExampleAppPageState extends State<Filter_CA> {
  ...
  final GlobalKey<FormState> _refresh = GlobalKey<FormState>();
  ...
  TextEditingController _PayeurController = TextEditingController();

  var loading = false;
  var dropdown_filiale;
  var dropdown_filiale1;
  var dropdown_choix;
  bool openaccording = false ;
  String filialSearch  ="0";
              ...
              Container(
                ...
                child: Column(
                  children: [
                   ...
                   child: Row(
                     children: [
                       Expanded(flex:4 ,child: dropdown_Payeur(PayeurController: _PayeurController)),
                       Expanded(
                           flex: 1,
                           child: Visibility(child: IconButton(onPressed: (){
  ...
  Widget buildElevatedButton(){
    return Container(
      ...
      child:  ElevatedButton(
        ...
        onPressed: () async {
          if(isLoading)return;
          if (_refresh.currentState!.validate()) {
            setState(() {
              isLoading = true ;
              Global.set_Payeur_Stat(_PayeurController.text)  ;
              Global.set_date_debut_Stat(dateinputFrom.text)  ;
              Global.set_date_fin_Stat(dateinputTo.text) ;
