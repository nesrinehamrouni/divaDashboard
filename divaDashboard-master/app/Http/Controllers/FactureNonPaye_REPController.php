...
class FactureNonPaye_REPController extends Controller
{
    function getFA_nonp_REP(request $req){
        ...
        if($req->ETB == "PLAST" )
            {
                $ETB ="  (derivedtbl_2.ETAB = 'PLAST') AND";
            }
        if($req->ETB == "MEUBLE" )
            {
                $ETB ="  (derivedtbl_2.ETAB = 'MEUBLE') AND ";
            }

        return DB :: select("SELECT     derivedtbl_2.REPR_0001, derivedtbl_2.NOMREP, SUM(derivedtbl_2.[MT FC]) AS MTT, SUM(derivedtbl_2.PAYE) AS PAYET, SUM(derivedtbl_2.RESTE) AS RESTET, derivedtbl_2.ETAB,SUM(ISNULL(derivedtbl_3.MTACOMPTE, 0)) AS ACOMPTE
        FROM        (SELECT     TOP (100) PERCENT REPR_0001, NOMREP, ETB, TIERS, NOM, FADT, PREFFANO, FANO, [MT FC], [MT FC] - MT AS PAYE, MT AS RESTE, Controle, 
                           ...
                           UNION ALL
                           SELECT     ENT.REPR_0001, VRP_1.NOM, ENT.ETB, ENT.TIERS, CLI_1.NOM AS CLI, ENT.PIDT, ENT.PREFPINO, ENT.PINO AS FANO, ENT.TTCMT, 0 AS PAYE, ENT.TTCMT AS RESTE, 
        ...
        GROUP BY derivedtbl_2.REPR_0001, derivedtbl_2.NOMREP, derivedtbl_2.ETAB
        HAVING   ".$ETB." (derivedtbl_2.REPR_0001 IN ('".$req->representant."'))") ;
    }


     function getdetailFA_REP(request $req){

        $ETB = null ;
    
        if($req->ETB = "PLAST" )
            {
                $ETB =" AND (derivedtbl_2.ETAB = 'PLAST')";
            }
        if($req->ETB = "MEUBLE" )
            {
                $ETB =" AND (derivedtbl_2.ETAB = 'MEUBLE')";
            }
       

        return DB::select("SELECT     derivedtbl_2.REPR_0001, derivedtbl_2.FADT, derivedtbl_2.NOMREP, { fn CONCAT(derivedtbl_2.PREFFANO, CONVERT(VARCHAR, derivedtbl_2.FANO)) } AS NUM_FC, derivedtbl_2.[MT FC], derivedtbl_2.PAYE, derivedtbl_2.RESTE, derivedtbl_2.Controle, derivedtbl_2.ETAB, 
        ISNULL(derivedtbl_3.MTACOMPTE, 0) AS ACOMPTE
FROM        (SELECT     TOP (100) PERCENT REPR_0001, NOMREP, ETB, TIERS, NOM, FADT, PREFFANO, FANO, [MT FC], [MT FC] - MT AS PAYE, MT AS RESTE, Controle, 
                           CASE WHEN ETB = 1 THEN 'MEUBLE' ELSE CASE WHEN ETB = 2 THEN 'MEUBLE' ELSE CASE WHEN PREFFANO IN ('22F', 'F') 
                           THEN 'MEUBLE' ELSE CASE WHEN FANO LIKE '230%' THEN 'MEUBLE' ELSE CASE WHEN PINOTIERS LIKE '%FH%' THEN 'PLAST' ELSE 'MEUBLE' END END END END END AS ETAB
         FROM        (SELECT     R2.R2_ID, R2.CE1, R2.CE3, R2.CE5, R2.DOS AS Expr1, R2.ETB, R2.G3CE1, R2.EFF, R2.TRANSAC, R2.TIERS, R2.ETAT, R2.ECHDT, R2.EMIDT, R2.EFFDT, R2.MT, LIENFACTURE.PREFFANO, LIENFACTURE.FANO, FACTURE.FADT, 
                               ...
                               CLI AS CLI_2 ON VRP_2.TIERS = CLI_2.REPR_0001 AND VRP_2.DOS = CLI_2.DOS RIGHT OUTER JOIN
                                   (SELECT     R2_1.R2_ID, R2_1.CE1, R2_1.CE3, R2_1.CE5, R2_1.DOS, R2_1.ETB, R2_1.G3CE1, R2_1.EFF, R2_1.TRANSAC, R2_1.TIERS, R2_1.ETAT, R2_1.ECHDT, R2_1.EMIDT, R2_1.EFFDT, - R2_1.MT AS MTACOMPTE, R2_1.USIM_ETB
                                    ...
                                    WHERE     (R2_1.DOS = ".$req->DOS.") AND (R2_1.CE1 = '2') AND (R2_1.CE5 = '1') AND (R2_1.CE3 = '1' OR
                                                      ...
                                                      R2_1.CE3 = '3') AND (R2_1.G3CE1 = 3) AND (R2_1.ETAT = 'WAR') AND (R2_1.EFFDT BETWEEN '".$req->date_debut."' AND '".$req->date_fin."')) AS derivedtbl_1_1 ON CLI_2.TIERS = derivedtbl_1_1.TIERS AND CLI_2.DOS = derivedtbl_1_1.DOS
             GROUP BY derivedtbl_1_1.TIERS, CLI_2.NOM, CLI_2.REPR_0001, VRP_2.NOM, CLI_2.FEU, derivedtbl_1_1.USIM_ETB) AS derivedtbl_3 ON derivedtbl_2.TIERS = derivedtbl_3.TIERS AND derivedtbl_2.ETAB = derivedtbl_3.ETAB
WHERE      (derivedtbl_2.REPR_0001 IN ('".$req->representant."')) AND (derivedtbl_2.FADT BETWEEN '".$req->date_debut."' AND '".$req->date_fin."') ".$ETB." ");
     }


    function get_Representant(request $req){
        return DB::select("SELECT TIERS,NOM from VRP WHERE DOS= ".$req->DOS." ");
    }
